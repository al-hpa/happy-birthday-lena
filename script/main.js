window.addEventListener('load', () => {
    ...
    const targetNode = document.querySelector('.text-box .fake-btn');
    ...
    const callback = function(mutationsList, observer) {
        for(let mutation of mutationsList) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                const backgroundColor = window.getComputedStyle(targetNode).backgroundColor;
                if (backgroundColor !== 'rgb(21, 161, 237)') {
                    document.querySelector('.cracker').style.visibility = 'visible';
                }
            }
        }
    };
...
const animationTimeline = () => {
    ...
    tl.to(".container", 0.6, { visibility: "visible" })
        ...
        .to(".three", 0.7, { opacity: 0, y: 10 }, "+=0.5")
        .from(".four", 0.7, { scale: 0.2, opacity: 0, })
        .from(".fake-btn", 0.3, { scale: 0.2, opacity: 0, })
        .staggerTo(".hbd-chatbox span", 1.5, { visibility: "visible", }, 0.05)
        .to(".fake-btn", 0.1, { backgroundColor: "rgb(127, 206, 248)", }, "+=0.5")
        .to(".four", 0.5, { scale: 0.2, opacity: 0, y: -150 }, "+=0.5")
        .add(() => {
            // Trigger fireworks when fake-btn disappears
            new Fireworks();
        })
        .from(".idea-1", 0.7, ideaTextTrans)
        .to(".idea-1", 0.7, ideaTextTransLeave, "+=2.5")
        .from(".idea-2", 0.7, ideaTextTrans)
        .to(".idea-2", 0.7, ideaTextTransLeave, "+=10.5")
        ...
        .to(".idea-5", 0.7, { scale: 0.2, opacity: 0, }, "+=0.5")
        .staggerFrom(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: 15, ease: Expo.easeOut, }, 0.2)
        .staggerTo(".idea-6 span", 0.8, { scale: 3, opacity: 0, rotation: -15, ease: Expo.easeOut, }, 0.2, "+=0.5")
        .staggerFromTo(".baloons img", 2.5, { opacity: 0.9, y: 1400, }, { opacity: 1, y: -1000, }, 0.2)
        .from(".profile-picture", 0.5, { scale: 3.5, opacity: 0, x: 25, y: -25, rotationZ: -45, }, "-=0.5")
        ...
        .staggerTo(".eight svg", 1.5, { visibility: "visible", opacity: 0, scale: 80, repeat: 3, repeatDelay: 1.4, }, 0.1)
